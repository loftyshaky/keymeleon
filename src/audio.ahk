...
play_audio(audio_type, layout := false, feature_state_bool := "", force_play := false) {
    ...
    enable_feature_state_audio := audio_type = "feature_state" && (check_if_need_to_play_audio("feature_state", false) || force_play)
    current_layout := layout ? layout : get_current_layout()
    custom_file_name := config_get(["audio", audio_type, enable_feature_state_audio ? feature_state_bool : current_layout])
...
check_if_need_to_play_audio(audio_type, is_automatic_layout_switching) {
    ...
    return is_automatic_layout_switching && !n(exe_enable_audio_final) || (!is_automatic_layout_switching && (!n(exe_enable_audio_final) && n(global_enable_audio) && global_enable_audio)) || n(exe_enable_audio_final) && exe_enable_audio_final
}
...
play_typing_audio(ih, vk, sc) {
    ...
    pressed_numpad_key_cond := pressed_numpad_calculation_character_key || (num_lock_is_on && pressed_numpad_key && !shift_held)

    if (check_if_need_to_play_audio(audio_type, false) && (pressed_key || pressed_numpad_key_cond) && !ctrl_held && !alt_held && !win_held) {
