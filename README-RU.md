...
| `delay_between`                 | `Number`       | Задержка в миллисекундах между симулируемыми состояниями `Down` и `Up` у `key`. Некоторые приложения реагируют мгновенно, другие вроде Fallout 4 требуют краткой задержки (обычно 30-50мс) для срабатывания имитированного нажатия. | `0`     |
| `pre_key_delay`                 | `Number`       | Задержка в миллисекундах между состоянием `Up` у `modifiers` и состоянием down у `key`. | `0`     |
| `post_key_delay`                 | `Number`       | Задержка в миллисекундах между состоянием `Up` у `key` и состоянием `Down` у `modifiers`. | `0`     |
...
| `allow_native_function` | `Number`       | Разрешить (`1`) или блокировать (`0`) исходную функцию клавиши. Например, установите `0`, чтобы предотвратить открытие меню "Пуск" при переназначении клавиши `Win`.                                                                                                                                   | `0`     |
| `ignore_extra_modifiers`      | `Number`       | Игнорировать (`1`) или учитывать (`0`) клавиши-модификаторы, уже удерживаемые при срабатывании привязки.                                                                                                                                                                                            | `0`     |
| `blind`      | `Number`       | Отправлять `key` с `{Blind}` (`1`) или без (`0`).                                                                                                                                                                                            | `0`     |
| `send_mode`      | `Number`       |Метод отправки для выполнения горячих клавиш. Возможные значения: `Send`, `SendInput`, `SendText`, `SendPlay`, `SendEvent`.                                                                                                                                                                                            | `SendInput`     |
